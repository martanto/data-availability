...
class SDS:
    ...
    def __init__(
        ...
        self.completeness: list[dict[str, Any]] = []
    ...
    def get_trace(self, date: datetime) -> Trace | None:
        ...
        if len(stream) > 1:
            ...
            raise ValueError(
                f"{date_str} :: Stream has more than one traces ({len(stream)} trace(s)). "
    ...
    def get_completeness(self, date: datetime) -> float:
        ...
        the expected daily sample count at the trace's sampling rate. Appends
        the result to :attr:`completeness`.
        ...
        if trace is None:
            self.completeness.append(completeness)
            if self.verbose:
        ...
        non_zero: float = np.round(
            ((np.count_nonzero(trace.data) / daily_sampling_rate) * sampling_rate), 2
        )

        if self.verbose:
            logger.info(f"{date_str} :: Completeness: {non_zero}%")

        completeness["completeness"] = non_zero
        self.completeness.append(completeness)
